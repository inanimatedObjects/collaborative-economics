...
/////// LINE CHART /////////
const marginLineChart = {top: 50, right: 20, bottom: 20, left: 70};
const widthLineChart = 450 - marginLineChart.right - marginLineChart.left;
const heightLineChart = 300 - marginLineChart.top - marginLineChart.bottom;
const circleRadius = 6;
// const width = 500 - marginLineChart.right - marginLineChart.left;
...
const xAxisBarChart = d3.axisBottom(xBarChart).ticks(0).tickSize(0),
        yAxisBarChart = d3.axisLeft(yBarChart).ticks(4).tickSize(1);
...
// d3.dsv(",", "bananaData.csv", convertTextToNumbers, function(d) {
d3.dsv(",", "bananaData.csv", function(d) {
// d3.csv("bananaData.csv", function(d) {
  return {
        year : new Date(+d.year, 5, 1), // convert year column to Date
        per : +d.per,
        date : +d.date,
        exclusiveEvents : +d.exclusiveEvents,
        exclusiveBal  : +d.exclusiveBal,
        exclCapex : +d.exclCapex,
        exclSavingsRate : +d.exclSavingsRate,
        exclBurn : +d.exclBurn,
        exclusiveNet : +d.exclusiveNet,
        exclusiveAcc : +d.exclusiveAcc,
        sharedEvents : +d.sharedEvents,
        sharedBal  : +d.sharedBal,
        sharedCapex : +d.sharedCapex,
        sharedSavingsRate : +d.sharedSavingsRate,
        sharedBurn : +d.sharedBurn,
        sharedNet : +d.sharedNet,
        sharedAcc : +d.sharedAcc,
        diffCapex : +d.diffCapex,
        diffSavingsRate : +d.diffSavingsRate,
        diffBurn : +d.diffBurn,
        diffNet : +d.diffNet,
        diffAcc : +d.diffAcc,
        // length: +d.Length // convert "Length" column to number
  };
}).then(function(dataStory) {
...
function changeCharts() {
        ...
        if (storyNextStep == storySteps.length ) {  //show the culumative upon final step
                ...
                const exclLifeEvent = storyGLineChart.selectAll(".life-event")
                .data(allStoryData)
                .enter()
                ...
                .attr("width", function() {
                  ...
                  const sharedLifeEvent = storyGLineChart.selectAll(".life-event")
                  .data(allStoryData)
                  .enter()
                  .filter(function (d) { return d.per == storySteps[storyNextStep].per ;});
