...
/////////////////////////////// CHART DIMENSIONS ///////////////////////////////////////////
/////// LINE CHART /////////
const margin = {top: 30, right: 0, bottom: 20, left: 80};
...
const heightBarChart = 500 - marginBarChart.top - marginBarChart.bottom;
...
d3.csv("bananaData.csv", convertTextToNumbers, function(error1, dataStory) {
        ...
        const timePeriods = d3.max(dataStory, (d) => d.date );
        console.log("***timePeriods :", timePeriods);
                // var timePeriods = d3.max([
                        // d3.max(dataStory, (d) => d.sharedDate ),
                        // d3.max(dataStory, (d) => d.exclusiveDate ),
                // ]);
        ...
        /////// LINE CHART /////////
        xLineChart.domain([ 0, 41 ]);    //set the domains using the loaded data
        ...
        var projectedData = [];
        var storyMaxdate = dataStory[dataStory.length-1].date;     // NM: combine/condense
...
///////////////////// DRAW BAR CHART //////////////////////////
        storydateData = allStoryData.filter((d) => { return d.date == storySteps[storyNextStep].date; } );
...
var textPosition;
if (xLineChart(d.date) > (widthLineChart* 0.9)) { textPosition = "end"; }
else if (xLineChart(d.date) < (widthLineChart* 0.1)) { textPosition = "start"; }
