...
const heightLineChart = 280 - verticalMargin;


const barMargin = {top: 40, right: 20, bottom: 30, left: 40};
const barWidth = 350 - barMargin.right - barMargin.left;
const barVerticalMargin = barMargin.top + barMargin.bottom;
const heightBarChart = 400 - barVerticalMargin;
...
// in order of how the story should flow. Make sure it aligns with the paragraph IDs in the HTML file.
...
const xAxisLineChart = d3.axisBottom(xLineChart)
        .tickSize(5);
const yAxisLineChart = d3
        .axisLeft(yLineChart)
        .ticks(6)
        .tickSize(2);
...
const xAxisBarChart = d3
        .axisBottom(xBarChart).ticks(4);

const yBarChart = d3.scaleLinear()
        .range([100, -100]);
        // .range([heightBarChart, 0]);
const yAxisBarChart = d3
        .axisLeft(yBarChart)
        .ticks(16)
        .tickSize(3);

////////////////////////////////////////////////
// const yAxisLineChart = d3.axisRight(yLineChart).ticks(10).tickSize(0);
...
.attr("barWidth", barWidth + margin.left + margin.right)
...
.attr("barWidth", function(d) { return Math.abs(xBarChart(d.exclusiveAcc) - xBarChart(0)); })
