...
const m = 4; // The number of values per series
...
const y = d3.scaleBand()
  ...
  .paddingOuter(0.38);          // padding before first and after last band
  // .padding(0.08);
...
// Each yzs[i] (TYPE) is an array of m time periods non-negative numbers representing a y-value for xz[i] (MONTHLY VALUE)
// The yzs array has n elements, representing the y-values of each of the n series
        // => yzs = [ jan, feb, ... , dec];

// The xz array has m elements, representing the x-values shared by all series
        // => xz = [  [ jan value, feb value, ..., dec value ],                  //  in use
                        // [ jan value, feb value, ..., dec value ] ,                 //  available
                        // [ jan value, feb value, ..., dec value ] ];                // unavailable/off hours

// The x01z array has the same structure as timePeriods, but with stacked [y₀, y₁] instead of y
        /* => x01z =  [
                [       starting_ xValue,  ending_xValue,                                // stacked bars
                        data: [  mVal_type1, mVal_type2, mVal_type3 ]            // non-stacked grouped
                ],
                [ feb arrays ],
                ...,
                [ dec arrays ]
